        ...
        def savingedit():
            if entry2.get() == entry3.get() == entry4.get() == entry5.get() == '':
                ...
                if car.nomer in baza:  # проверка на наличие уже такого номера в базе и занесение в словарь
                    baza[car.nomer] = (car.get_data_bez_nomera()[0],
                                       car.get_data_bez_nomera()[1],
...
def edit_win_str():
    ...
    def savingedit():
        if entry2.get() == entry3.get() == entry4.get() == entry5.get() == '':
            ...
            car = Car(stroka[0], entry2.get(), entry3.get(), entry4.get(), entry5.get())
            if car.nomer in baza:  # проверка на наличие уже такого номера в базе и занесение в словарь
                baza[car.nomer] = (car.get_data_bez_nomera()[0],
                                   car.get_data_bez_nomera()[1],
                                   car.get_data_bez_nomera()[2],
    ...
    elif stroka[0] in baza:
        marka, tip, sostoyanie, cvet = baza[stroka[0]]
        edit_autowin = tk.Tk()
        edit_autowin.title('')
...
#     baza = pickle.load(f)
baza = {"AA300": ["car", "car", "car", "car",],
        "AA304": ["car", "car", "car", "car",]}

main_win = tk.Tk()
main_win.iconbitmap('github.ico')
main_win.title('каталог автомобилей')


main_win.resizable(False, False)
table = Table(main_win, headings=('НОМЕР', 'МАРКА', 'ТИП', 'СОСТОЯНИЕ', 'ЦВЕТ'),
              rows=((key,value[0],value[1],value[2], value[3]) for key, value in baza.items()))
